...
## 3.1 前提: Calico をアンインストール済みであること

Step 2.15 を完了していること。

```bash
kubectl get pods -n calico-system 2>/dev/null | head -3
...
[2.15](02-calico.md#215-calico-のアンインストール-次の-cni-に進む場合) で
CoreDNS Pod を削除済みなら `Pending` のまま待っているはずです。Cilium が
Running になった時点で、追加の操作なしに自動的にスケジュールされ、新しい
IP が払い出されます。
...
**もし 2.15 で削除し忘れていた場合**は、ここで手動削除してください
(Calico 時代の古い IP のままだと `CrashLoopBackOff` します):
...
`nginx-ds` / `debug` も 2.15 で削除済みの前提です。Step 1/2 と同じ manifest
...
**もし前章の Pod がまだ残っている場合**は、古い Calico IP を持っているため
...
# Cilium のアンインストール (必要な場合)
helm uninstall cilium -n kube-system
```

[04-comparison.md](04-comparison.md) に進みます。
