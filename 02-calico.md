...
## 2.1 前提: Flannel をアンインストール済みであること

Step 1.10 を完了していること。

```bash
kubectl get pods -n kube-flannel 2>/dev/null | head -3
# Error from server (NotFound): ... または No resources found
```
...
[01.10](01-flannel.md#110-flannel-のアンインストール-次の-cni-に進む場合) で
CoreDNS Pod を削除済みなら `Pending` のまま待っているはずです。Calico が
Running になった時点で、追加の操作なしに自動的にスケジュールされ、Calico の
IPAM から新しい IP が払い出されます。
...
**もし 01.10 で削除し忘れていた場合**は、ここで手動削除してください
(古い Flannel IP のままだと `CrashLoopBackOff` します):
...
`nginx-ds` / `debug` も 1.10 で削除済みの前提です。Step 1 と同じ manifest
(`manifests/nginx-ds.yaml`) を使って作り直します:

```bash
kubectl apply -f manifests/nginx-ds.yaml
```

**もし Step 1 の Pod がまだ残っている場合**は、古い Flannel IP を持っているため
...
## 2.15 Calico のアンインストール (次の CNI に進む場合)
...
**⚠️ 重要**: [01.10](01-flannel.md#110-flannel-のアンインストール-次の-cni-に進む場合)
と同じ理由で、CoreDNS Pod は Calico のインターフェース/CNI 設定を削除した
**後**に削除しています。こうすることで CoreDNS は `Pending` のまま待機し、
03 章で Cilium が起動した瞬間に自動的にスケジュールされ、新しい IP を取得
します。ここで削除を忘れると、Calico 時代の古い IP を持ったまま残り
Cilium 導入後に `CrashLoopBackOff` します。

[03-cilium.md](03-cilium.md) に進みます。
