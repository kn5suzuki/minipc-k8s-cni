...
`manifests/nginx-ds.yaml` (nginx DaemonSet + ClusterIP Service + `debug` という busybox Pod) は3つの CNI の章すべてで同一のまま使い回すテスト用ワークロードであり、これにより挙動を公平に比較できる。`manifests/iperf3.yaml` (iperf3 の client/server Pod 3つ、worker1/worker1/worker2 に固定) は 02-calico.md・03-cilium.md がそれぞれ持つ「トンネリング vs ネイティブルーティング」性能比較のセクションで使い回す。
